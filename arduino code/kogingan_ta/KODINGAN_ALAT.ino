...
//     abort();
//   }
//   rtc.adjust(DateTime(2025, 5, 4, 15, 30, 0)); // (tahun, bulan, tanggal, jam, menit, detik)
...
const long scheduleCheckInterval = 60000;
...
void processScheduleData(String scheduleData) {  
  ...
  if (firstPos > 0 && secondPos > firstPos && thirdPos > secondPos) {
    ...
    showNotification("Schedule Update", String(todayUserSchedule.scheduleHour) + ":" + String(todayUserSchedule.scheduleMinute), 2000);
  }
}
...
void setup() {
  ...
  rtc.adjust(DateTime(2025, 5, 3, 17, 37, 0));
  lcWadah.begin(LOADCELL_WADAH_DOUT_PIN, LOADCELL_WADAH_SCK_PIN);
  lcWadah.set_scale(calibration_factor_wadah);
  lcWadah.tare();
  pinMode(echoPinWadah, INPUT);
